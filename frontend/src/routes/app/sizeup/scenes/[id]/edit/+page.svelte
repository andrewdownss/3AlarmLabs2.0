<script lang="ts">
	...
	async function handleSave() {
		...
		try {
			...
			if (payload?.type === 'failure') {
				saveError = payload?.data?.formError ?? 'Save failed.';
				return;
			}
			saveSuccess = true;
			posthog.capture('scene_saved', {
				scene_id: data.scene.id,
				scene_title: data.scene.title,
	...
	async function generateHiRes() {
		...
		try {
			const fd = new FormData();
			const res = await fetch('?/generateHiRes', {
				method: 'POST',
				body: fd,
			...
			isUpgradingImage = false;
		}
	}

	$effect(() => {
		overlays = normalizeAnimationOverlays(data.scene.overlaysJson as AnimationOverlay[] | null);
	});

	onMount(() => {
		if (data.needsHiRes) generateHiRes();
	});
</script>
...
<div class="mx-auto w-full max-w-[1600px] px-4 py-8">
	<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
		<div class="min-w-0">
			...
			<p class="mt-1 text-sm text-muted-foreground">
				Drag fire or smoke animations onto the scene, then save.
			</p>
		</div>
		<div class="flex flex-wrap gap-2 sm:justify-end">
			...
			<Button
				variant="outline"
				href={`/app/sizeup/scenes/new/capture?sceneId=${encodeURIComponent(data.scene.id)}`}
				>Recapture</Button
			>
	...
	{#if baseImageSrc}
		<KonvaOverlayEditor
			baseImageUrl={baseImageSrc}
			initialOverlays={overlays}
			on:overlaysChange={(e: CustomEvent<{ overlays: AnimationOverlay[] }>) =>
				(overlays = e.detail.overlays)}
		/>
	{:else}
		<div class="flex h-96 items-center justify-center rounded-xl border bg-muted/30">
			<p class="text-sm text-muted-foreground">
				No image captured. Go back and capture a Street View image first.
			</p>
		</div>
	{/if}
</div>
