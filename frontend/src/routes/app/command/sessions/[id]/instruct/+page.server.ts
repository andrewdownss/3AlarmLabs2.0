...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	const session = await db.query.trainerSessions.findFirst({ where: eq(trainerSessions.id, params.id) });
	if (!session) throw redirect(303, '/app/command');
	if (session.instructorId !== locals.user.id && session.studentId !== locals.user.id) {
		throw redirect(303, '/app/command');
	}
	const [scenario, boardEntries] = await Promise.all([
		db.query.trainerScenarios.findFirst({ where: eq(trainerScenarios.id, session.scenarioId) }),
