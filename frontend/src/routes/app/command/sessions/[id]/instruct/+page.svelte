<script lang="ts">
	...
	function addTimelineEvent(type: string, text: string) {
		timelineEvents = [
			...timelineEvents,
			{ id: crypto.randomUUID(), type, text, time: formatClock(sessionSeconds) }
		];
	}
	...
	function dispatch() {
		...
		if (draftStage !== currentStage)
			addTimelineEvent('STAGE', `Dispatched stage: ${stageLabels[draftStage]}`);
		if (draftSide !== currentSide)
			addTimelineEvent('SIDE', `Dispatched side: ${sideLabels[draftSide]}`);
		...
		<div class="flex flex-1 items-center justify-center p-6">
			<div class="w-full max-w-lg rounded-2xl border bg-card p-8 text-center shadow-sm">
				...
				<p class="mt-2 text-sm text-muted-foreground">
					Share this 5-character code with a student.
				</p>
				<div class="mt-6 rounded-xl border bg-muted/40 px-4 py-4 sm:px-6 sm:py-5">
					<p
						class="text-center font-mono text-2xl font-bold tracking-[0.35em] sm:text-4xl sm:tracking-[0.4em]"
					>
						{joinCode || '-----'}
					</p>
				</div>
	...
	{:else if !hasStarted}
		<div class="flex flex-1 items-center justify-center p-6">
			<div class="w-full max-w-lg rounded-2xl border bg-card p-8 text-center shadow-sm">
				...
				<p class="mt-2 text-sm text-muted-foreground">
					The student has joined and is waiting for you to begin.
				</p>
		...
		<div class="flex min-h-0 flex-1 flex-col overflow-hidden lg:flex-row">
			<main class="order-1 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:order-none">
				<div class="flex shrink-0 flex-wrap items-center gap-2 border-b px-3 py-2 sm:px-4">
					<div
						class="relative h-16 w-full max-w-[11rem] shrink-0 overflow-hidden rounded-lg border bg-black sm:h-20 sm:w-40 sm:max-w-[10rem]"
					>
					...
					<div class="space-y-0.5 text-xs">
						...
						<Badge class="text-[10px] {stageBadgeClass[currentStage] ?? 'bg-gray-500'} text-white"
							>{stageLabels[currentStage] ?? currentStage}</Badge
						>
					</div>
				</div>
				...
				<div class="min-h-0 flex-1 space-y-3 overflow-y-auto p-3 sm:p-4">
					<div class="rounded-xl border bg-card">
						<div
							class="flex flex-col gap-1 border-b px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:px-4"
						>
						...
						<div class="border-b bg-muted/30 px-3 py-2.5 sm:px-4">
							<div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end">
								<div class="w-full sm:w-32">
									<label class="mb-1 block text-[10px] text-muted-foreground" for="assign-unit"
										>Unit</label
									>
									<select
										id="assign-unit"
										bind:value={assignUnit}
										class="h-11 w-full rounded-md border bg-background px-2 text-xs focus:ring-2 focus:ring-ring focus:outline-none sm:h-8"
									>
								...
								<div class="w-full sm:w-36">
									<label class="mb-1 block text-[10px] text-muted-foreground" for="assign-division"
										>Column</label
									>
									<select
										id="assign-division"
										bind:value={assignDivision}
										class="h-11 w-full rounded-md border bg-background px-2 text-xs focus:ring-2 focus:ring-ring focus:outline-none sm:h-8"
									>
										<option value="">Select...</option>
								...
								<div class="w-full sm:w-40">
									<label
										...
										for="assign-assignment">Assignment</label
									>
									<select
										id="assign-assignment"
										bind:value={assignAssignment}
										class="h-11 w-full rounded-md border bg-background px-2 text-xs focus:ring-2 focus:ring-ring focus:outline-none sm:h-8"
									>
										<option value="">Select...</option>
								...
								<Button
									...
									onclick={assignUnitToBoard}
									disabled={!assignUnit || !assignDivision}>Assign</Button
								>
							</div>
						</div>
						...
						<div class="px-1 py-2 sm:px-2 sm:py-3">
							<div class="overflow-x-auto pb-1 lg:overflow-x-hidden lg:pb-0">
								<div class="flex w-max gap-1.5 lg:w-full lg:min-w-0">
									{#each COMMAND_BOARD_COLUMNS as col (col.key)}
										<div
											class="flex min-h-0 w-[5.25rem] shrink-0 flex-col border bg-muted/15 sm:w-28 lg:w-0 lg:min-w-0 lg:flex-1"
										>
											<div
												class="flex min-h-[2.25rem] items-center justify-center border-b bg-muted/40 px-1 py-1.5 text-center text-[10px] font-bold tracking-tight text-muted-foreground uppercase"
											>
												{col.header || '\u00a0'}
											</div>
											<div class="flex max-h-56 min-h-0 flex-col gap-1 overflow-y-auto p-1.5">
												{#each entriesForColumn(boardEntries as BoardEntryLike[], col.key) as entry (entry.id ?? entry.unitName)}
													<div
														class="group rounded-md border px-1.5 py-1 text-[10px] leading-tight {STATUS_COLORS[
															entry.status
													...
													>
														...
														<div class="mt-1 flex items-center gap-0.5">
															<select
																...
																class="h-5 min-w-0 flex-1 cursor-pointer rounded border-0 bg-transparent px-0 text-[9px] font-medium focus:ring-1 focus:ring-ring focus:outline-none"
															>
															...
															<button
																...
																class="shrink-0 text-red-600 opacity-60 hover:opacity-100"
																aria-label="Remove"
															>
																<svg
																	xmlns="http://www.w3.org/2000/svg"
							...
							<div class="border-t px-3 py-2 sm:px-4">
								...
								<div class="flex flex-wrap gap-1">
									{#each legacyBoardEntries as entry (entry.id ?? entry.unitName)}
										<span
											class="rounded border px-2 py-0.5 text-[10px] {STATUS_COLORS[entry.status] ??
												'bg-gray-50'}"
										>
			...
			<aside
				class="order-2 flex max-h-[min(42vh,420px)] min-h-[220px] w-full shrink-0 flex-col overflow-hidden border-t border-border bg-background lg:order-none lg:max-h-none lg:min-h-0 lg:w-80 lg:border-t-0 lg:border-l"
			>
				<div class="flex shrink-0 border-b">
					<button
						type="button"
						onclick={() => (activeInstructorTab = 'controls')}
						class="flex-1 px-3 py-2.5 text-xs font-medium transition-colors {activeInstructorTab ===
						'controls'
					...
					<button
						type="button"
						onclick={() => (activeInstructorTab = 'board')}
						class="flex-1 px-3 py-2.5 text-xs font-medium transition-colors {activeInstructorTab ===
						'board'
							? 'border-b-2 border-primary bg-background'
							: 'bg-muted/30 text-muted-foreground hover:text-foreground'}"
				...
				<div class="flex-1 space-y-3 overflow-y-auto p-3">
					{#if activeInstructorTab === 'controls'}
						<div class="space-y-2 rounded-lg border p-3">
							...
							{#if draftSideImage}
								<div
									class="relative w-full overflow-hidden rounded-lg border bg-black"
							...
							<div class="space-y-1.5">
								<div class="flex flex-wrap gap-1">
									{#each ['incipient', 'growth', 'fully_developed', 'decay'] as stage (stage)}
										<button
											type="button"
											onclick={() => (draftStage = stage)}
											class="rounded px-2 py-0.5 text-[10px] font-medium transition-colors {draftStage ===
											stage
								...
								<div class="flex flex-wrap gap-1">
									{#each ['alpha', 'bravo', 'charlie', 'delta'] as side (side)}
										<button
											type="button"
											onclick={() => (draftSide = side)}
											class="rounded px-2 py-0.5 text-[10px] font-medium transition-colors {draftSide ===
											side
												? 'bg-primary text-primary-foreground'
												: 'bg-muted text-muted-foreground hover:bg-accent'}"
						...
						<div class="space-y-2 rounded-lg border p-3">
							...
							<div class="relative">
								<div class="flex flex-col gap-2 sm:flex-row sm:gap-1.5">
									<Input
										bind:value={hazardText}
										placeholder="Collapse, backdraft..."
										class="h-11 min-h-11 text-xs sm:h-8 sm:min-h-0"
										onfocus={() => (showHazardSuggestions = true)}
									/>
									<Button
										size="sm"
										variant="outline"
										class="h-11 w-full shrink-0 sm:h-8 sm:w-auto"
										onclick={() => injectHazard()}>Inject</Button
									>
								</div>
								{#if showHazardSuggestions}
									...
									>
										{#each SUGGESTED_HAZARDS as suggestion (suggestion)}
											<button
												type="button"
												class="w-full px-3 py-1.5 text-left text-xs transition-colors hover:bg-accent"
												onclick={() => injectHazard(suggestion)}>{suggestion}</button
											>
										{/each}
									</div>
									<button
										...
										onclick={() => (showHazardSuggestions = false)}
										aria-label="Close"
									></button>
								{/if}
							</div>
							...
							<div class="relative">
								<div class="flex flex-col gap-2 sm:flex-row sm:gap-1.5">
									<Input
										...
										placeholder="Water supply, PAR..."
										class="h-11 min-h-11 text-xs sm:h-8 sm:min-h-0"
										onfocus={() => (showUpdateSuggestions = true)}
									/>
									<Button
										size="sm"
										variant="outline"
										class="h-11 w-full shrink-0 sm:h-8 sm:w-auto"
										onclick={() => sendUpdate()}>Send</Button
									>
								</div>
									...
									>
										{#each SUGGESTED_UPDATES as suggestion (suggestion)}
											<button
												type="button"
												class="w-full px-3 py-1.5 text-left text-xs transition-colors hover:bg-accent"
												onclick={() => sendUpdate(suggestion)}>{suggestion}</button
											>
										{/each}
									</div>
									<button
										type="button"
										class="fixed inset-0 z-40"
										onclick={() => (showUpdateSuggestions = false)}
										aria-label="Close"
									></button>
								{/if}
							</div>
							...
							<div class="max-h-40 space-y-1.5 overflow-y-auto">
								{#each studentTranscripts as t (t.id)}
									<div class="flex gap-1.5 text-xs">
										<span class="font-mono text-muted-foreground">{t.time}</span><span
											>{t.transcript}</span
										>
									</div>
								{:else}
						...
						<div>
							...
							<div class="max-h-96 space-y-1.5 overflow-y-auto">
								{#each timelineEvents as event (event.id)}
									<div class="flex gap-1.5 text-xs">
										<span class="shrink-0 font-mono text-muted-foreground">{event.time}</span><Badge
											variant="outline"
