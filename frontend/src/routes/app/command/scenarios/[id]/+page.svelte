<script lang="ts">
	...
	const initialSelfPacedConfig = untrack(
		() => data.scenario.selfPacedConfigJson as SelfPacedConfig | null
	);
	...
	function timelineAxisBucketSeconds(longestRelevantSeconds: number): number {
		const bucketSeconds = 3 * 60;
		const expansionBufferSeconds = 30;
		const longest = clampNonNegInt(longestRelevantSeconds);
		const bucketCount = Math.max(
			1,
			Math.floor((longest + expansionBufferSeconds) / bucketSeconds) + 1
		);
		return bucketCount * bucketSeconds;
	}
	...
	const timelineAxisMaxSeconds = $derived.by(() => {
		...
		const limit =
			selfPacedConfig.timeLimitSeconds != null
				...
				: 0;
		return timelineAxisBucketSeconds(Math.max(limit, ...offsets, ...deadlines, 0));
	});

	function scrollToTimelineEvent(eventId: string) {
		document
			.getElementById(`selfpaced-tl-${eventId}`)
	...
	function initOverlays(): SideStageOverlays {
		...
		for (const side of SIDES) {
			...
			for (const stage of STAGES) {
				...
				result[side.key][stage.key] = normalizeAnimationOverlays(
					persisted as unknown as AnimationOverlay[]
				);
			}
		}
		return result;
...
<div class="mx-auto w-full max-w-7xl px-4 py-10">
	...
	<div class="space-y-6">
		<div
			class="rounded-xl border border-primary/30 bg-primary/5 px-4 py-3 text-sm leading-relaxed text-foreground dark:bg-primary/10"
		>
			...
			<ol class="mt-1.5 ml-4 list-decimal space-y-0.5 text-xs text-muted-foreground sm:text-sm">
				<li>
					Fill in <strong class="font-medium text-foreground">Details</strong> (title, dispatch notes,
					construction, etc.).
				</li>
				<li>
					Upload building images and add fire/smoke overlays in <strong
						class="font-medium text-foreground">Scene Builder</strong
					>.
				</li>
				<li>
					Add apparatus in <strong class="font-medium text-foreground">Default Resources</strong> so students
					have units to assign.
				</li>
				<li>
					<a href="#self-paced-script" class="font-medium text-primary underline underline-offset-2"
						>Scroll to Self-Paced Script</a
					>
					and toggle <strong class="font-medium text-foreground">Enable self-paced mode</strong> on if
					you want a scripted, interactive timeline (otherwise the scenario runs as free-form self-practice).
				</li>
				<li>
					Click <strong class="font-medium text-foreground">Save Changes</strong>. Students run it
					from the Command page via <em>Self Practice</em> or <em>Instructor-Led</em>.
				</li>
			</ol>
		</div>
				...
				<form id="details-form" class="space-y-4">
					...
					<div class="space-y-1.5">
						...
						<textarea
							...
							>{data.scenario.description ?? ''}</textarea
						>
					</div>
					<div class="space-y-1.5">
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div class="space-y-1.5">
							...
							>
								...
								{#each ['Type I - Fire Resistive', 'Type II - Non-Combustible', 'Type III - Ordinary', 'Type IV - Heavy Timber', 'Type V - Wood Frame'] as t (t)}
						...
						<div class="space-y-1.5">
							...
							>
								<option value="">Select...</option>
								{#each ['1st Alarm', '2nd Alarm', '3rd Alarm', 'All Hands'] as a (a)}
					...
					<div class="grid gap-4 sm:grid-cols-2">
						...
						<div class="space-y-1.5">
							...
							<Input
								id="occupancyType"
								name="occupancyType"
								value={data.scenario.occupancyType ?? ''}
							/>
						</div>
					</div>
				</form>
			...
			<Card.Root>
				<Card.Header>
					...
					<p class="text-sm text-muted-foreground">
						Publish this scenario as a premade self-paced simulation for paid users.
					</p>
				</Card.Header>
				<Card.Content>
					<form method="POST" action="?/publishToLibrary" class="space-y-4">
						<label class="flex items-start gap-3 rounded-lg border bg-muted/30 px-4 py-3 text-sm">
							<input
								type="checkbox"
								name="isLibrary"
								class="mt-1"
								checked={data.scenario.isLibrary}
							/>
							<span>
								<span class="font-medium text-foreground">Publish to weekly library</span>
								<span class="mt-1 block text-muted-foreground">
									Library scenarios become read-only catalog content for non-admins and can only be
									run in Self Practice.
								</span>
							</span>
						</label>
						<div class="space-y-1.5">
							<label class="text-sm font-medium" for="publishedAt">Publish date</label>
							<Input
								id="publishedAt"
								name="publishedAt"
								type="datetime-local"
								value={toDateTimeLocal(data.scenario.publishedAt)}
							/>
							<p class="text-xs text-muted-foreground">Leave blank to publish immediately.</p>
						</div>
		...
		<Card.Root>
			<Card.Header>
				...
				<p class="text-sm text-muted-foreground">
					Upload building images and add fire/smoke overlays per side and stage.
				</p>
			</Card.Header>
			<Card.Content class="space-y-4">
				<div class="flex gap-1 rounded-lg border bg-muted/30 p-1">
					{#each SIDES as side (side.key)}
						<button
							...
							class="flex-1 rounded-md px-3 py-2 text-sm font-medium transition-colors {activeSide ===
							side.key
					...
					<div class="flex-1 space-y-1">
						...
						{#if !activeImageUrl}
							<p class="text-xs text-muted-foreground">
								Upload an image to begin adding overlays for this side. Max 10 MB.
							</p>
						{/if}
						{#if uploadError}
						...
						<div class="flex gap-1" role="group" aria-labelledby="stage-label">
							{#each STAGES as stage (stage.key)}
								...
								<button
									...
									class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors {activeStage ===
									stage.key
								...
								>
									...
									{#if count > 0}
										<span
											class="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-primary-foreground/20 text-[10px]"
											>{count}</span
										>
									{/if}
								</button>
				...
				{:else}
					<div
						class="flex h-64 items-center justify-center rounded-xl border-2 border-dashed bg-muted/20"
					>
						<p class="text-sm text-muted-foreground">
							Upload an image for {activeSideConfig.label} to start building overlays.
						</p>
					</div>
				{/if}
			</Card.Content>
				...
				<div class="flex flex-wrap gap-2">
					{#each data.scenario.defaultResources as resource (resource.unitName)}
						<Badge variant="secondary" class="gap-1.5">
							...
							<button
								type="button"
								onclick={() => removeResource(resource.unitName)}
								class="ml-1 text-muted-foreground hover:text-destructive">&times;</button
							>
						</Badge>
					{/each}
				</div>
		...
		<Card.Root id="self-paced-script">
			<Card.Header>
				...
				<p class="text-sm text-muted-foreground">
					Turn this on to make the scenario <strong class="font-medium text-foreground"
						>scripted and interactive</strong
					>. You define timed events (stage changes, hazards, dispatch updates), expected actions,
					and end conditions (time limit, "under control" declaration, etc.). When students open the
					scenario they'll see a "Start scenario" gate, and the timeline runs automatically after
					they press start. Leave it off to run the scenario as free-form self-practice.
				</p>
			</Card.Header>
			<Card.Content class="space-y-6">
				<div class="flex items-center justify-between rounded-lg border bg-muted/20 px-4 py-3">
					<div>
						...
						<p class="text-xs text-muted-foreground">
							{#if selfPacedEnabled}
								On: the scenario is scripted. Configure the timeline and end conditions below, then
								press <strong class="font-medium text-foreground">Save self-paced script</strong>.
							{:else}
								Off: the scenario runs as free-form self-practice with no scripted events. Turn this
								on to author timed dispatches and end conditions.
							{/if}
						</p>
					</div>
				...
				{#if selfPacedEnabled}
					...
					>
						...
						<div class="border-t border-border/60 px-4 pt-1 pb-4">
							<div class="grid gap-4 pt-3 sm:grid-cols-2">
								<div class="space-y-1.5">
									...
									<div class="flex flex-wrap items-end gap-2">
										...
										<div class="space-y-1">
											<span class="text-xs text-muted-foreground" id="timeLimitSec-h"
												>Seconds (0–59)</span
											>
											<Input
												id="timeLimitSec"
									...
									<p class="text-xs text-muted-foreground">
										{#if selfPacedConfig.timeLimitSeconds != null}
											= <strong class="font-medium text-foreground"
												>{formatDurationShort(selfPacedConfig.timeLimitSeconds)}</strong
											>
								...
								<div class="space-y-1.5">
									...
									<label class="flex items-center gap-2 text-sm">
										<input
											type="checkbox"
											bind:checked={selfPacedConfig.endConditions.onTimelineComplete}
										/>
									...
									<label class="flex items-center gap-2 text-sm">
										<input
											type="checkbox"
											bind:checked={selfPacedConfig.endConditions.onTimeExpired}
										/>
									...
									<label class="flex items-center gap-2 text-sm">
										<input
											type="checkbox"
											bind:checked={selfPacedConfig.endConditions.onUnderControl}
										/>
