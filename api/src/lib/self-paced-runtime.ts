...
async function loadDispatchedTimelineIds(
  sessionId: string,
): Promise<Set<string>> {
  const rows = await db
...
async function loadCompletedExpectedIds(
  sessionId: string,
): Promise<Set<string>> {
  const rows = await db
    .select({
  ...
  for (const r of rows) {
    if (
      r.eventType === "expected_action_completed" ||
 ...
 */
export async function runTimelineTick(
  io: Server,
  sessionId: string,
  now: Date = new Date(),
): Promise<void> {
...
async function dispatchedAllTimelineIds(
  sessionId: string,
...
async function fireDueScheduledEvents(
  io: Server,
  session: RuntimeSession,
  now: Date,
): Promise<void> {
 ...
 */
export async function evaluateAfterBoardChange(
  io: Server,
  sessionId: string,
): Promise<void> {
  const session = await loadSession(sessionId);
  ...
  await markExpectedActionsFromBoard(
    sessionId,
    config.expectedActions,
    board,
    elapsedSec,
  );
  await scheduleAssignmentCompletions(
    sessionId,
    config.assignmentCompletions,
    board,
    new Date(),
  );
...
async function markExpectedActionsFromBoard(
  ...
  for (const action of actions) {
    ...
    const isLate =
      typeof action.deadlineSeconds === "number" &&
