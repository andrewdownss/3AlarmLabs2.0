...
const KEY_SET = new Set(BOARD_COLUMN_KEYS);
/** True if value is already a canonical column key. */
export function isBoardColumnKey(value) {
...
export function normalizeBoardColumn(parsed) {
    ...
    if (!raw)
        return null;
    if (isBoardColumnKey(raw))
        return raw;
    const t = raw.toLowerCase().replace(/\s+/g, ' ');
    const ifMatch = (patterns, key) => patterns.some((p) => t === p || t.includes(p));
    if (ifMatch(['basement', 'below grade', 'cellar'], 'Basement'))
        return 'Basement';
    if (ifMatch(['div 1', 'division 1', 'div1', 'd1', 'first division', 'floor 1', '1st floor'], 'Div 1'))
        return 'Div 1';
    ...
    if (ifMatch(['roof', 'rooftop', 'on the roof', 'aerial', 'vertical vent', 'ladder pipe', 'roof operations'], 'Roof'))
        return 'Roof';
    if (ifMatch(['ric', 'rapid intervention', 'rit', 'fast team'], 'RIC'))
        return 'RIC';
    if (ifMatch(['med', 'medical', 'ems', 'rehab'], 'Med'))
        return 'Med';
