 ...
 */
export function parseBetterAuthSessionCookieHeader(header) {
    const cookies = cookie.parse(header || '');
    const secure = cookies[SECURE_SESSION_COOKIE];
    if (secure)
        return { cookieName: SECURE_SESSION_COOKIE, raw: secure };
    const plain = cookies[PLAIN_SESSION_COOKIE];
    if (plain)
        return { cookieName: PLAIN_SESSION_COOKIE, raw: plain };
    return null;
}
/**
