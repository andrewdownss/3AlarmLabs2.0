...
export function createSelfPacedRouter(io) {
    const router = Router();
    ...
    router.post('/:id/pause', async (req, res) => {
        ...
        if (session.pausedAt) {
            res.json({ success: true, alreadyPaused: true });
            return;
        }
    ...
    router.post('/:id/resume', async (req, res) => {
        ...
        await db
            ...
            .where(and(eq(trainerScheduledEvents.sessionId, id), isNull(trainerScheduledEvents.firedAt)));
        await db
            .update(trainerSessions)
            .set({
            pausedAt: null,
    ...
    router.post('/:id/board/correct', async (req, res) => {
        const id = String(req.params.id);
        const session = await getSessionForUser(id, req.userId);
        if (!session) {
            res.status(404).json({ error: 'Not found' });
        ...
        const existing = await db
            ...
            .where(and(eq(trainerCommandBoardEntries.sessionId, id), eq(trainerCommandBoardEntries.unitName, data.unitName)))
            .limit(1);
        ...
        if (data.radioMessageId) {
            await db
                ...
                .where(and(eq(trainerRadioMessages.id, data.radioMessageId), eq(trainerRadioMessages.sessionId, id)));
        }
 ...
 */
export function startSelfPacedPoller(io, intervalMs = 2000) {
    const tick = async () => {
        try {
            const now = new Date();
            const dueScheduled = await db
                ...
                .where(and(isNull(trainerScheduledEvents.firedAt), lte(trainerScheduledEvents.fireAt, now)));
